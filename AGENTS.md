...
- `crates/provider`: protocol-neutral capabilities, types, values, provider/operation/parameter schemas, calls, results, errors, `ServerProvider`, and `ProviderRegistry`. Its native feature owns the built-in HTTP, direct-process, and sandboxed-filesystem providers. It never parses the DSL or depends on browser, editor, or adapter crates.
...
- `crates/app-bridge`: Protocol 1 manifest/schema/hash validation and generated wire DTOs/codecs. Its native feature adds bounded framing, authenticated transports, lifecycle, compatibility adapters, and the shared `AppProvider`; portable schema and wire support remain available without native features.
...
- `crates/observation`: execution IDs/events and revision-bound runtime observations stored by `(FileId, SourceRevision)`, including structured failures, value diffs, page/candidate evidence, and repair hints.
...
- `crates/editor`: protocol-neutral document state, diagnostic composition, formatting, semantic tokens, hover/completion/signature services, and run orchestration. It returns internal DTOs, never LSP, DAP, or VS Code types.
...
Production dependencies point from adapters toward reusable cores. Keep `text`, `feedback`, `provider`, and `project` independent of their consumers; forbid `syntax -> hir/analysis/plan/runtime/editor/adapters`, `hir -> analysis/plan/runtime/editor/adapters/browser-cdp`, `plan -> syntax/analysis/runtime/adapters`, `analysis -> project/runtime/browser/browser-cdp/editor/adapters`, `browser -> browser-cdp`, `runtime -> syntax/analysis/project/editor/app`, any runtime dependency on CLI/report formatting, and `editor -> LSP/DAP types or browser-cdp`. Test-only dev-dependencies may cross a boundary for vertical or black-box verification, but do not authorize the same production coupling. `app` is the composition root and may depend on all native components.
