...
The Milestone E foundation now represents each test and capability body as an explicit plan `Sequence`, with structurally derived declaration/node identities independent of runtime occurrence IDs. The sequential runner traverses that tree and emits parented scope/operation facts, holding the test root open through test cleanup. Plan format 9 and runtime semantics 6 version the execution tree independently and fingerprint resolved project inputs; CLI report/event schema 9 includes scope, retry-attempt, and attachment facts with recorded journal metadata, sequence, and clocks. Runtime observations commit atomically and older in-flight runs cannot overwrite newer batches. The resource registry now enforces acquisition acknowledgement, shared/exclusive leases, generations, and terminal teardown. Browser contexts use the generic resource/wait driver; scope cancellation, inherited deadlines, and a separate cleanup budget have deterministic fake-clock coverage. RunControl can cancel active waits, and direct processes and application command adapters share native capture that explicitly interrupts and reaps on cancellation. Active bridge calls await terminal cancellation acknowledgement; bridge transport shutdown owns its reader and fallback cancellation tasks. `timeout` is available in every capability domain with local bindings, inherited deadlines, precise timeout origins, and lexical temporary-directory teardown. Discarded provider temporary-directory results are owned and cleaned through the resource registry. `parallel` now uses independent branch state, explicit parentage, lexical browser contexts, prompt primary-failure signalling, and ordered typed branch aggregates. Transferable snapshots and generic resource-access analysis reject unsafe captures and inherited exclusive browser access. Public `race`/`provide` now use a distinct Race plan node and the same sibling driver for first-success selection, typed loser cancellation, awaited cleanup, and recovered observation filtering. Bound races require compatible transferable results, carry the winner value with its redaction metadata, and mark the final winning branch in structured reports. Public `retry` now lowers to the same owned-child execution used by parallel/race, with fresh attempt identities, repeatability validation, cancellation-aware capped backoff, and separate attempt evidence. Compiler diagnostics reject unsafe operations and native captures; browser blocks inside retry own fresh contexts while inherited observation-only browser work reuses its explicit enclosing context. `webtest test --jobs N` now admits isolated test roots across files through a separate runtime scheduler (1–64, default 1), retaining slots through teardown and source-ordered final reports. Owned applications now run once per worker, receiving `WEBTEST_APP_URL`/`WEBTEST_APP_PORT` and `WEBTEST_WORKER_ID`, with matching bridge/HTTP/browser routing and one active root per worker. Concurrent roots own separate browser sessions and observation batches; early infrastructure failures stop admission in the affected file. Native journal schema 6 records carry per-execution sequence identity, separate clocks, source revision/origin, and explicit optional execution context, with bounded idempotent replay validation. Complete records serialize through the shared observation types into flat JSON envelopes; bounded replay decoding checks version, identity, and source/context agreement before insertion. Retry attempt start/finish facts include local ordinals and limits, with terminal outcomes and cancellation after teardown. Branch-local operation metadata survives delayed failure processing; the collector never infers a current sibling scope. Native journals now enforce per-file event-count budgets configured by `[journal].max_events`, cancel unhealthy roots while awaiting cleanup, and retain explicit loss intervals and a reserved terminal record. Bounded asynchronous subscribers end with typed gap markers without affecting execution. Acknowledged evidence writes emit attachment facts immediately with byte length and BLAKE3 digest; failed or expired writes produce no false attachment references. Artifact DTOs live in observation and remain re-exported by runtime. File-run identities use native OS randomness and serialize as 32 lowercase hexadecimal digits across processes; allocation failure is typed infrastructure before test admission and produces no fabricated identity or journal records. Complete host/resource coverage, remaining event kinds, traces, IPC, and concurrent DAP are still pending; `specs/milestone-e.md` records that boundary.

Do not claim unimplemented roadmap features—such as modules, user-defined functions, actionability, retries, traces, or CLI-to-LSP IPC—already exist. Add them incrementally without weakening the shared architecture.
...
- `crates/host`: protocol-neutral operation-context and cancellation contracts shared by providers, browser hosts, observations, and runtime. It owns no scheduler or host mechanics; runtime implements cancellation propagation and deadlines.
...
- `crates/runtime`: structured plan execution, provider dispatch, plan-to-browser conversion, evidence/artifact capture, structured results/events/failures/diffs/repair hints, observation recording, and the pre-step `RunControl` hook used by DAP. It does not parse source or print terminal output.
