...
- `crates/app-bridge`: protocol-1 manifest/schema validation, generated wire DTOs, bounded framing, authenticated transports, lifecycle, compatibility adapters, and the shared `AppProvider`. Portable schema code remains available without native features.
