...
The Milestone E foundation now represents each test and capability body as an explicit plan `Sequence`, with structurally derived declaration/node identities independent of runtime occurrence IDs. The sequential runner traverses that tree and emits parented scope/operation facts, holding the test root open through test cleanup. Plan format 4 separates format/runtime-semantics versions and fingerprints resolved project inputs; CLI report/event schema 4 includes scope facts. Runtime observations commit atomically and older in-flight runs cannot overwrite newer batches. The general resource/cancellation scheduler, concurrent control constructs, jobs, authoritative event journal, traces, IPC, and concurrent DAP are still pending; `specs/milestone-e.md` records that boundary.
