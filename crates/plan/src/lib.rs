...
mod retry;
pub use retry::{
    MAX_RETRY_ATTEMPTS, RetryBackoff, RetryPolicy, RetrySafetyViolation, RetrySettings,
};
mod tree;
pub use resources::{
    ResourceAccess, ResourceConflict, ResourceReference, ResourceUse, conflicting_resource_accesses,
};
pub use tree::{
    MAX_CONTROL_TIMEOUT, MAX_PARALLEL_BRANCHES, ParallelFailurePolicy, PlanNode, PlanNodeKind,
    PlanTreeError, ResourcePlan, declaration_identity,
};
mod compatibility;
pub use compatibility::{PlanCompatibilityError, PlanExecutionInputs};

pub const PLAN_FORMAT_VERSION: u32 = 9;
pub const RUNTIME_SEMANTICS_VERSION: u32 = 6;
...
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct ProvideOperation {
    pub expression: PlanExpr,
    pub result_type: Type,
}

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct RaceBinding {
    pub id: BindingId,
    pub name: String,
    pub ty: Type,
}
...
mod tests {
    ...
    fn envelope_is_versioned_serializable_and_rejects_unknown_versions() {
        ...
        for version in [1, 2, 3, 4, 5, 6, 7, 8, PLAN_FORMAT_VERSION + 1] {
